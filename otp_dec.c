...
void error(const char *msg, int x) { perror(msg); exit(x); } // Error function used for reporting issues
...
int main(int argc, char *argv[])
{
	...
	char text[100000], key[100000]; //stores data to send to otp_dec_d and store data sent back
	int cipherTextFD, keyFD; //file descriptors for inputs
	int numChars = 0, i;
	char nextC[1], nextK[1];
	...
	// Get ciphertext from file, store in array
	memset(text, '\0', sizeof(text)); // Clear out the text array
	//open cipher text file
	cipherTextFD = open(cipherFile, O_RDONLY);
	if(cipherTextFD < 0)
		error("ERROR opening ciphertext file\n", 1);
	//read one character at a time from ciphertext into array, checking for invalid characters
	while(read(cipherTextFD, nextC, sizeof(char))){
		if(*nextC == '\n')//stop reading at newline char- anything after will be ignored
			break;
		text[numChars] = *nextC; //store character in array
		...
		if( !(text[numChars] == 32 || (text[numChars] >= 65 && text[numChars] <= 90)) ){ //if invalid char
			fprintf(stderr, "Error at char number %d: '%d'\n", numChars, text[numChars]);
			error("ERROR: invalid characters in ciphertext file\n", 1);
		}
		numChars++;
	}
	close(cipherTextFD);	
	...
	//get plaintext from server, read until termination character '$' found
